...
export function getApiEndpoint() {
  const row = getDb().prepare("SELECT value FROM settings WHERE key = 'api_endpoint'").get();
  return row?.value || API_ENDPOINTS[0];
}
...
export function setApiEndpoint(endpoint) {
  ...
  getDb().prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('api_endpoint', ?)").run(endpoint);
}
...
// 带重试的 fetch
async function fetchWithRetry(urlPath, options, maxRetries = 3) {
  let lastError;

  // 如果是完整 URL（图片下载等），直接请求
  if (urlPath.startsWith('http')) {
    for (let i = 0; i < maxRetries; i++) {
      try {
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), 60000);
        const resp = await fetch(urlPath, { ...options, signal: controller.signal });
        clearTimeout(timeout);
        return resp;
      } catch (err) {
        lastError = err;
...
generateRouter.post('/generate', async (req, res) => {
  ...
  try {
    ...
    const resp = await fetchWithRetry('/v1/images/generations', {
      method: 'POST',
      headers: {
...
generateRouter.get('/tasks/:id', async (req, res) => {
  ...
  try {
    const resp = await fetchWithRetry(`/v1/tasks/${id}`, {
...
async function pollTask(taskId, apikey) {
  ...
  try {
    ...
    const resp = await fetchWithRetry(`/v1/tasks/${taskId}`, {
